...
listTumors = ['BLCA', 'BRCA', 'CESC', 'COADREAD', 'ESCA', 'GBM', 'HNSC', 'LGG', 'PRAD', 'SARC', 'STAD', 'TGCT', 'UCEC']
...
for tumor in listTumors:
    ...
    print('Parse tumor type/subtypes')
    def parse_types(fn):
        pat2type = {}
        pat2subtype = {}
        with open(fn) as f:
    ...
    training_set = pd.read_table('data/'+folderSomaticAlt+'/'+tumor+'/'+tumor+'_alterations_training.tsv', index_col=0)


    # Fisher test against types (Optional)
    print('Fisher tests')
    training_pat2type = training_set.index.to_series().map(pat2subtype)
    gene2fisherp = {}
    ...
    print('Output files')
    edge2features_df.to_csv('data/'+folderSomaticAlt+'/'+tumor+'/'+tumor+'_edge2features.txt', sep='\t', header=False, index=False)

    with open('data/'+folderSomaticAlt+'/'+tumor+'/'+tumor+'_feature_names.txt', 'w') as f:
